...
contract CashswapLocker is Ownable, ReentrancyGuard {
  ...
  ICashswapFactory public cashswapFactory;
  ...
  mapping(address => TokenLock[]) public tokenLocks; //map cashswap pair to all its locks
  ...
  struct FeeStruct {
    ...
    uint256 liquidityFee; // fee on cashswap liquidity tokens
  ...
  constructor(ICashswapFactory _cashswapFactory) public {
    devaddr = msg.sender;
  ...
  /**
   * @notice set the migrator contract which allows locked lp tokens to be migrated to cashswap next version
   */
   ...
   * @param _lpToken the cashswap token address
   * @param _amount amount of LP tokens to lock
   * @param _unlock_date the unix timestamp (in seconds) until unlock
   * @param _referral the referrer address if any or address(0) for none
   * @param _fee_in_eth fees can be paid in eth or in a secondary token such as CSWAP with a discount on cashswap tokens
  ...
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _referral, bool _fee_in_eth, address payable _withdrawer) external payable nonReentrant {
    ...
    // ensure this pair is a cashswap pair by querying the factory
    ICashswapPair lpair = ICashswapPair(address(_lpToken));
    ...
    // record the lock for the cashswappair
  ...
  function relock (address _lpToken, uint256 _index, uint256 _lockID, uint256 _unlock_date) external nonReentrant {
    ...
    // send cashswap fee to dev address
    TransferHelper.safeTransfer(_lpToken, devaddr, liquidityFee);
  }
  ...
  /**
   * @notice migrates liquidity to cashswap next version
   */
